...
def add_random_bg_planes(bg_images, max_num=5):
    def add_one(bottom_angle = 3.14/6):
        bottom_locs = [np.random.uniform(-0.1, 0.1), np.random.uniform(-0.1, 0.1), np.random.uniform(-0.2, 0)]
...
def create_box(bg_images, cc_textures):
    ...
    w_delta = -0.1
    room_planes = [bproc.object.create_primitive('CUBE', scale=[x_w, y_w, 0.01]),
...
def generate_rendering():
    ...
    for i, obj in enumerate(sampled_bop_objs, start = 1):
        obj.set_cp('obj_id', i)  # 设置实例 ID 为对象在列表中的索引
