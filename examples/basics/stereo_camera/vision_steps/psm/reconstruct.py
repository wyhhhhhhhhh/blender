...
# model_file = os.path.join(current_script_directory, "checkpoint_299.tar")
model_file = os.path.join(current_script_directory, "checkpoint_220.tar")
...
roi = False
mask_pixels = [(265, 110), (463, 94), (480, 252), (279, 268)] 
...
def disparity_to_depth(disparity_map, focal_length, baseline):
    ...
    zero_depth_condition = (disparity_map == 0)

    depth_map[~zero_depth_condition] = (focal_length * baseline) / disparity_map[~zero_depth_condition]
...
def inference(imgL,imgR):
    ...
    # Condition to set disparity to 0
    zero_depth_condition = (pred_disp == 0) | ((x_coords - pred_disp) < 0)
    pred_disp[zero_depth_condition] = 0
