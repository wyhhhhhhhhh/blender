...
class BopWriterUtility:
    ...
    def _get_frame_camera(save_world2cam, depth_scale=1.0, unit_scaling=1000., destination_frame=["X", "-Y", "-Z"]):
        ...
        cam_pose = WriterUtility.get_cam_attribute(bpy.context.scene.camera, 'cam2world_matrix',
                                                                  local_frame_change=destination_frame)
        cam_pose = np.array(cam_pose)
        print("cam_pose: ", cam_pose)
        print("cam_pose_inv_R: ", cam_pose[:3, :3].T)
        print("cam_pose_inv_t: ", -cam_pose[:3, :3].T @ cam_pose[:3, 3])
        ...
        for frame_id in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            if colors:
                ...
                if color_file_format == 'PNG':
                    ...
                    # cv2.imwrite("testin-rgb.png", color_rgb)
                    # cv2.imwrite("testin-bgr.png", color_bgr)
                    # # save gray image converted from rgb image
                    # cv2.imwrite(rgb_fpath, color_rgb)
